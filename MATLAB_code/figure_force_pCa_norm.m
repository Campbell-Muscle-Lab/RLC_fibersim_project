...
for cond_counter = 1: numel(conditions_to_draw)
    ...
    hsl_values(cond_data_folder) = unique(summary.hsl)

    vi = find(summary.hsl == hsl_values(cond_data_folder))

    pd(cond_data_folder).pCa = summary.pCa(vi)
    pd(cond_data_folder).y = summary.force(vi)/(max(summary.force(vi)))
    pd(cond_data_folder).y_error = 0 * pd(folder_counter).y;

    [pCa50(cond_data_folder),n(cond_data_folder),~,~,~, pd(cond_data_folder).x_fit, pd(cond_data_folder).y_fit] = ...
        fit_Hill_curve(pd(cond_data_folder).pCa, pd(cond_data_folder).y);
